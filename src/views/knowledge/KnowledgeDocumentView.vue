...
function downloadCurrentDoc() {
  ...
  if (!id) return
  const link = document.createElement('a')
  const baseUrl = previewDocData.value?.fileDownloadUrl || `/api/knowledge/documents/${id}/file`
...
function getFileToken(): string {
  return auth.accessToken || localStorage.getItem('access_token') || localStorage.getItem('geocms_token') || ''
}

function getMediaFileUrl(file: any): string {
  if (!file || !file.id) return ''
  const token = getFileToken()
  const baseUrl = `/api/knowledge/documents/${file.id}/file`
